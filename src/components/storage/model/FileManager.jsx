...
function FileManager({ show, handleClose, handleFiles }) {
  ...
  const handleDeleteFile = (lastModified) => {
    const filters = selectedFiles?.filter(
          ...
          <div className="file-list mt-1">
            {selectedFiles?.length > 0 &&
              selectedFiles?.map((file, index) => (
                <div
                  key={index}
                  className="card border border-danger mb-1 p-1 d-flex justify-content-between "
                >
                  <p
                    style={{
                      margin: 0,
                  ...
                  <span>
                    <Button
                      variant="danger"
                      size="sm"
                      onClick={() => handleDeleteFile(file?.lastModified)}
                    >
                      <RiDeleteBack2Fill />
                    </Button>
                  </span>
